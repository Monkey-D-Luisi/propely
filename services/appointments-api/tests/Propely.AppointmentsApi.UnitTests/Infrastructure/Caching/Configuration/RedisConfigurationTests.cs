...
namespace Propely.AppointmentsApi.UnitTests.Infrastructure.Caching.Configuration;
...
public sealed class RedisConfigurationTests
{
    ...
    public void Defaults_ShouldBeSetCorrectly()
    {
        ...
        config.KeyPrefix.Should().Be("appointmentsapi:");
    }
    ...
    public void ShouldImplementICacheSettings()
    {
        ...
        config.Should().BeAssignableTo<Propely.AppointmentsApi.Application.Common.Interfaces.ICacheSettings>();
        ((Propely.AppointmentsApi.Application.Common.Interfaces.ICacheSettings)config).DefaultTtlMinutes.Should().Be(5);
    }
}

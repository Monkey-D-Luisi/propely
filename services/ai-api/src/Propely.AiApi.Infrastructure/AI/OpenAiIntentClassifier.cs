...
public sealed class OpenAiIntentClassifier : IIntentClassifier
{
    ...
    public async Task<ClassifiedIntent> ClassifyAsync(string text, IReadOnlyList<ConversationExchange>? history = null, CancellationToken ct = default)
    {
        ...
        // NOTE: ResultMessage content is system-generated by action handlers, not raw user input.
        // However, if user-controlled data is reflected in result messages (e.g., entity names),
        // there is an indirect prompt injection risk. Future mitigation: sanitize result messages
        // or use a structured format that separates data from instructions.

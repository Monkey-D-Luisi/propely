...
| Service | Path | Tech | Port | Purpose |
|---------|------|------|------|---------|
| web | `apps/web/` | Next.js 16 + Tailwind CSS | 3000 | Frontend |
| ai-api | `services/ai-api/` | .NET 10 | 5010 | AI capabilities, content generation |
| orgs-api | `services/orgs-api/` | .NET 10 | 5020 | Auth, agencies, branches, billing, permissions |
| properties-api | `services/properties-api/` | .NET 10 | 5030 | Core property domain |
| publishing-api | `services/publishing-api/` | .NET 10 | 5040 | Portal feeds, webhooks |
| contacts-api | `services/contacts-api/` | .NET 10 | 5050 | Contacts, leads |
| appointments-api | `services/appointments-api/` | .NET 10 | 5060 | Scheduling, calendar sync |
...
# Infrastructure only (Postgres, RabbitMQ, Redis, etc.)
...
# Orgs API on port 5020
./scripts/run-orgs-api.sh
.\scripts\run-orgs-api.ps1

# Properties API on port 5030
./scripts/run-properties-api.sh
.\scripts\run-properties-api.ps1

# Publishing API on port 5040
./scripts/run-publishing-api.sh
.\scripts\run-publishing-api.ps1

# Contacts API on port 5050
./scripts/run-contacts-api.sh
.\scripts\run-contacts-api.ps1

# Appointments API on port 5060
./scripts/run-appointments-api.sh

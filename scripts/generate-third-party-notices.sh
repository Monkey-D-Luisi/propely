...
SERVICE_SRC_DIRS=(
    "$REPO_ROOT/services/ai-api/src"
    "$REPO_ROOT/services/orgs-api/src"
    "$REPO_ROOT/services/properties-api/src"
    "$REPO_ROOT/services/publishing-api/src"
    "$REPO_ROOT/services/contacts-api/src"
    "$REPO_ROOT/services/appointments-api/src"
)
DEP_HASHES=$(find "${SERVICE_SRC_DIRS[@]}" -name '*.csproj' | sort | xargs sha256sum 2>/dev/null || find "${SERVICE_SRC_DIRS[@]}" -name '*.csproj' | sort | xargs shasum -a 256)

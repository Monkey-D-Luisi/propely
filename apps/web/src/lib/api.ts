...
const APPOINTMENTS_API_BASE = process.env.NEXT_PUBLIC_APPOINTMENTS_API_URL ?? "http://localhost:5070";
...
/**
 * Fetch from the Contacts API with automatic token refresh on 401.
 */
export async function contactsApiFetch<T>(
  path: string,
  init: RequestInit = {},
  schema?: { parse: (data: unknown) => T },
): Promise<T> {
